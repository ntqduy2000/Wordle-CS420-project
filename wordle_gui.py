...
def main():
    ...
    # ---------------------------------------------------------------------
    # load ALL words (now serves as both answers and allowed words)

    # all_answers is renamed to all_words for clarity
    all_words = []
    # NOTE: You MUST create a single file named 'data_files/all_words.txt' 
    # containing all unique words from the old all_answers.txt and allowed_words.txt.
    try:
        with open("data_files/all_words.txt", "r") as f:
            # Use a set to ensure unique words, then convert back to list
            # list(set(...)) is generally safer if the source file isn't guaranteed unique
            all_words = list(set([line.upper().rstrip() for line in f]))
            all_words.sort() # Optional: keep the list ordered
    ...
    # !!! The answer is now picked from the combined 'all_words' list !!!
    # Ensure word_tracker doesn't exceed the new list length
    if stats["word_tracker"] >= len(all_words):
        stats["word_tracker"] = 0
        
    answer = all_words[stats["word_tracker"]]

    # inc tracker# if > max_length, reset tracker
    stats["word_tracker"] += 1
    if stats["word_tracker"] >= len(all_words): # Use len(all_words) for the maximum size
        stats["word_tracker"] = 0
    ...
    while True:
        ...
        # elif enter and do have five letters: join into string and check if valid
            ...
            # Check against the single 'all_words' list
